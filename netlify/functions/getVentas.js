export async function handler(event) {
  ...
  try {
    ...
    const urlVentas =
      `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/Ventastotal!A:AB?key=${API_KEY}`;

    const resVentas = await fetch(urlVentas);

    if (!resVentas.ok) {
      throw new Error("Error al leer Ventastotal");
    }

    const dataVentas = await resVentas.json();

    if (!dataVentas.values) {
      throw new Error("Ventastotal vacía");
    }
    ...
    const urlGastos =
      `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/Gastos!A:J?key=${API_KEY}`;
